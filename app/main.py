...
from .report import summarize_events, compute_integrity_score, write_html_report
...
async def upload_video(session_id: str, file: UploadFile = File(...)):
    ...
    save_path = VIDEO_DIR / f"{session_id}{suffix}"
    content = await file.read()
    save_path.write_bytes(content)

    await sessions_collection.update_one(
        {"_id": session_id},
        {"$set": {"video_path": str(save_path)}}
    )
...
async def get_report(session_id: str):
    ...
    # Generate HTML report to disk for optional viewing
    write_html_report(session_obj, counts, integrity_score, REPORT_DIR)
